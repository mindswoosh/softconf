...
sub SmartFixName {
    ...
    # Remove all "'s from the name. They cause Trouble!
    # $name =~ s/\"//g;       # Commented out because this line breaks hyperlinks...

    #  Correct some unnecessary abbreviations (these are really spell checking corrections)
    $name =~ s/\bExec([s]?)\./Executive$1/ig;        # Exec. -> Executive  and  Execs. -> Executives
    $name =~ s/\bDir([s]?)\./Director$1/ig;          # Dir.  -> Director   and  Dirs.  -> Directors

    #  Handle some positional titles
    $name =~ s/\bSnr\./Sr./ig;
    $name =~ s/\bceo\b/CEO/ig;                  # CEO
    $name =~ s/\bcfo\b/CFO/ig;                  # CFO
    $name =~ s/\bcoo\b/COO/ig;                  # COO
    $name =~ s/\bvp\b/VP/ig;                    # VP
    $name =~ s/\brvp\b/RVP/ig;                  # RVP
    $name =~ s/\bsvp\b/SVP/ig;                  # SVP
    $name =~ s/\bgvp\b/GVP/ig;                  # GVP
    $name =~ s/\bnvp\b/NVP/ig;                  # NVP
	$name =~ s/\bbvp\b/BVP/ig;                  # BVP
	$name =~ s/\brgm\b/RGM/ig;                  # RGM

    #  Now handle some directional abbreviations
    $name =~ s/\bne\b/NE/ig;                    # NE
    $name =~ s/\bnw\b/NW/ig;                    # NW
    $name =~ s/\bse\b/SE/ig;                    # SE
    $name =~ s/\bsw\b/SW/ig;                    # SW
    
    #  Don't do anything if the name has BOTH upper and lowercase letters
    return $name   if ($name =~ /[A-Z]+/  &&  $name =~ /[a-z]+/);
    ...
    $name =~ s/([\s\-]Mc)(.)/$1\U$2/g;          # McNames
    $name =~ s/([\s\-]Mac)(.)/$1\U$2/g;         # MacNames
    $name =~ s/([\s\-]O')(.)/$1\U$2/g;          # O'Names
    $name =~ s/([\s\-]Le)(.)eu/$1\U$2\Leu/g;    # LeFeuvre, LeBeuf, etc.
